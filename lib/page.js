...
var Page = function( page_path, options ){
	...
	this.fetchResource = function( resource_data, params, callback ){
		...
		var req = hyperquest( resource_url, resource_data, function( err, res ){
			if( err ) return callback( err, null );
			// log a warning if the response code doesn't look like a success
			if( SUCCESSFUL_RESPONSE_CODES.indexOf( res.statusCode ) < 0 ){
				server.logger.log( 'Possible resource failure with status code '+ res.statusCode +': '+ resource_data.url, 3 );
			}
			var data = '';
			...
			res.on( 'end', function(){
				...
				cache.set( resource_data.url, response );
