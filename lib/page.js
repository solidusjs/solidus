	...
	this.parsePage = function( callback ){
		...
		fs.readFile( this.path, DEFAULT_ENCODING, function( err, data ){
			...
			try {
				...
				if( solidus.options.log_level >= 1 ) console.log('[SOLIDUS]'.red.bold +' Error preprocessing "'+ page.path +'"', err );
			}
			finally {
				page.params = params;
				_( page ).extend({
		...
		if( preprocessor ){
			...
			fs.readFile( preprocessor_path, DEFAULT_ENCODING, function( err, data ){
				try {
					...
					if( solidus.options.log_level >= 1 ) console.error( '[SOLIDUS]'.red.bold +' Error in preprocessor: '+ preprocessor_path, err );
				}
				finally {
